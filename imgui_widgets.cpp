...
#ifdef IMGUI_USE_OPENCV_HSV_VALUE
#define IM_F32_TO_INT8_HUE_UNBOUND(_VAL) ((int)((_VAL) * 179.0f + ((_VAL)>=0 ? 0.5f : -0.5f))) // Unsaturated, for display purpose, but without rounding to nearest integer (OpenCV HSV value is truncated, not rounded)
#define IM_F32_TO_INT8_HUE_SAT(_VAL)    ((int)(ImSaturate(_VAL) * 179.0f + 0.5f))               // Saturated, always output 0..179
#else
#define IM_F32_TO_INT8_HUE_UNBOUND(_VAL) IM_F32_TO_INT8_UNBOUND(_VAL)
#define IM_F32_TO_INT8_HUE_SAT(_VAL)    IM_F32_TO_INT8_SAT(_VAL)
#endif //IMGUI_USE_OPENCV_HSV_VALUE
//-------------------------------------------------------------------------
// Warnings
//-------------------------------------------------------------------------
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    bool isHsv = flags & ImGuiColorEditFlags_InputHSV;
    ...
    int i[4] = { isHsv ? IM_F32_TO_INT8_HUE_UNBOUND(f[0]) : IM_F32_TO_INT8_UNBOUND(f[0]), IM_F32_TO_INT8_UNBOUND(f[1]), IM_F32_TO_INT8_UNBOUND(f[2]), IM_F32_TO_INT8_UNBOUND(f[3]) };
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            int idx = (n + ((n & 1) ? 0 : Shif_BGR)) % 4;
            if (draw_color_marker)
                SetNextItemColorMarker(GDefaultRgbaColorMarkers[idx]);

            // FIXME: When ImGuiColorEditFlags_HDR flag is passed HS values snap in weird ways when SV values go below 0.
            if (flags & ImGuiColorEditFlags_Float)
            {
                value_changed |= DragFloat(ids[idx], &f[idx], 1.0f / 255.0f, 0.0f, hdr ? 0.0f : 1.0f, fmt_table_float[fmt_idx][idx], drag_flags);
                value_changed_as_float |= value_changed;
            }
            else
            {
                value_changed |= DragInt(ids[idx], &i[idx], 1.0f, 0, hdr ? 0 : 255, fmt_table_int[fmt_idx][idx], drag_flags);
            }
            if (!(flags & ImGuiColorEditFlags_NoOptions))
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        char buf[64];
        if (alpha)
            ImFormatString(buf, IM_COUNTOF(buf), "#%02X%02X%02X%02X", ImClamp(i[idxs[0]], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[idxs[2]], 0, 255), ImClamp(i[3], 0, 255));
        else
            ImFormatString(buf, IM_COUNTOF(buf), "#%02X%02X%02X", ImClamp(i[idxs[0]], 0, 255), ImClamp(i[1], 0, 255), ImClamp(i[idxs[2]], 0, 255));
        ...
        if (InputText("##Text", buf, IM_COUNTOF(buf), ImGuiInputTextFlags_CharsUppercase))
        {
            ...
            int r;
            if (alpha)
                r = sscanf(p, "%02X%02X%02X%02X", (unsigned int*)&i[idxs[0]], (unsigned int*)&i[1], (unsigned int*)&i[idxs[2]], (unsigned int*)&i[3]); // Treat at unsigned (%X is unsigned)
            else
                r = sscanf(p, "%02X%02X%02X", (unsigned int*)&i[idxs[0]], (unsigned int*)&i[1], (unsigned int*)&i[idxs[2]]);
...
#ifdef IMGUI_USE_OPENCV_HSV_VALUE
            f[0] = (flags & ImGuiColorEditFlags_DisplayHSV) ? (i[0] / 179.0) : (i[0] / 255.0);
        for (int n = 1; n < 4; n++)
            f[n] = i[n] / 255.0f;
#else
        for (int n = 0; n < 4; n++)
...
#ifdef IMGUI_USE_BGRA_PACKED_COLOR
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text("#%02X%02X%02X\nB: %d, G: %d, R: %d\n(%.3f, %.3f, %.3f)", cb, cg, cr, cb, cg, cr, col[2], col[1], col[0]);
        else
            Text("#%02X%02X%02X%02X\nB:%d, G:%d, R:%d, A:%d\n(%.3f, %.3f, %.3f, %.3f)", cb, cg, cr, ca, cb, cg, cr, ca, col[2], col[1], col[0], col[3]);
#else
        if (flags & ImGuiColorEditFlags_NoAlpha)
...
#endif // IMGUI_USE_BGRA_PACKED_COLOR
        int cr = ((flags & ImGuiColorEditFlags_DisplayHSV) ? IM_F32_TO_INT8_HUE_SAT(col[idxs[0]]) : IM_F32_TO_INT8_SAT(col[idxs[0]])), cg = IM_F32_TO_INT8_SAT(col[1]), cb = IM_F32_TO_INT8_SAT(col[idxs[2]]), ca = (flags & ImGuiColorEditFlags_NoAlpha) ? 255 : IM_F32_TO_INT8_SAT(col[3]);
        char buf[64];
        ImFormatString(buf, IM_COUNTOF(buf), "(%.3ff, %.3ff, %.3ff, %.3ff)", col[idxs[0]], col[idxs[1]], col[idxs[2]], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : col[3]);
        if (Selectable(buf))
